<?php
    ...
    function insert_product() {
        ...
        if(mysqli_query($connection, $query)) {
            ...
            $response=array(
                'status' => 0,
                'status_message' => 'Plant Addition Failed.'
            );
        }
    ...
    function delete_product($product_id) {
        ...
        if(mysqli_query($connection, $query)) {
            $response=array(
                'status' => 1,
                'status_message' => 'Plant Deleted Successfully.'
            );
        } else {
            $response=array(
                'status' => 0,
                'status_message' => 'Plant Deletion Failed.'
            );
        }
        header('Content-Type: application/json');
        echo json_encode($response);
    ...
    function update_product($product_id) {
        ...
        if(mysqli_query($connection, $query)) {
            $response=array(
                'status' => 1,
                'status_message' => 'Plant Updated Successfully.'
            );
        } else {
            $response=array(
                'status' => 0,
                'status_message' => 'Plant Updation Failed.'
            );
        }
        header('Content-Type: application/json');
        echo json_encode($response);
